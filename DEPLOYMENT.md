...
## 14. Руководство по миграции на Chatbot API 2.0 (Fetch Mode)

Для миграции с устаревшего пользовательского опроса сообщений (user-scoped polling) на новый событийный бот-опрос (bot-scoped fetch loop) выполните следующие шаги:

1. **Удалите или перерегистрируйте старого бота:**
   Отзовите регистрацию старого бота в Битрикс24 (если он был зарегистрирован в режиме обычного чат-бота).

2. **Зарегистрируйте нового бота в режиме `supervisor` с типом `fetch`:**
   Вызовите REST-метод `imbot.v2.Bot.register` со следующим JSON-телом (например, через встроенный REST-клиент Битрикс24):
   ```json
   {
     "code": "tg_mirror_bot",
     "type": "supervisor",
   ...
   ```
   *Примечание:* Поле `handler_url` при регистрации указывать не нужно, так как бот работает в режиме получения событий через fetch-запросы.

3. **Обновите переменные окружения:**
   В файле `.env` (или `/opt/bitrix-bot/.env`) укажите:
   - `BITRIX_BOT_ID` — числовой ID, полученный при регистрации бота.
   - `BITRIX_BOT_CLIENT_ID` — токен бота (`botToken`), полученный при регистрации.
   - `BITRIX_WEBHOOK_BRIDGE_ENABLED=false` — внутренний мост-вебхук больше не используется.
...
5. **Подключите чат к зеркалированию:**
   - Добавьте нового зарегистрированного бота в нужный групповой чат Битрикс24.
   - В чате напишите команду `/tg_connect` — бот пришлет одноразовый токен авторизации.
   - Перейдите в соответствующую Telegram-группу и отправьте команду `/connect <dialog_id> <token>` для восстановления связки.

6. **Проверка работоспособности:**
   Убедитесь в логах (`journalctl -u bitrix-telegram-mirror.service -f`), что fetch-опрос событий (`imbot.v2.Event.get`) успешно активен, события зеркалируются, и отсутствуют ошибки 403 от старых пользовательских методов `im.dialog.messages.get`.
