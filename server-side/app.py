...
BITRIX_CLIENT_ID = os.getenv('BITRIX_CLIENT_ID', '').strip() or os.getenv('BITRIX_BOT_CLIENT_ID', '').strip()
...
FORWARDED_EVENTS = {item.strip() for item in os.getenv('BITRIX_FORWARDED_EVENTS', 'ONIMBOTMESSAGEADD,ONIMBOTJOINCHAT').split(',') if item.strip()}
_SECRET_PATTERNS = re.compile('(auth|token|password|secret|webhook|key|pwd|access_token|refresh_token)([\'\\"]?\\s*[:=]\\s*[\'\\"]?)([^\\s\'\\"&,;}{)\\]]{4,})', re.IGNORECASE)
...
async def send_bot_message(dialog_id: str, message: str, bot_id: str=''):
    ...
    payload = {'botId': int(final_bot_id), 'botToken': BITRIX_CLIENT_ID, 'dialogId': dialog_id, 'fields': {'message': message, 'urlPreview': False}}
    ...
    try:
        if event == 'ONIMBOTJOINCHAT' and dialog_id:
            ...
            if reply is not None:
                await send_bot_message(dialog_id=dialog_id, bot_id=bot_id, message=reply)
