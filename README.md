...
Проект решает практическую задачу: сотрудники могут общаться в привычном Telegram-чате, а сообщения, правки, реакции и часть вложений синхронизируются с соответствующим диалогом в Битрикс24. В обратную сторону сервис опрашивает события методом imbot.v2.Event.get (Chatbot API 2.0 Fetch Mode) для надёжной последовательной доставки событий из Bitrix в Telegram.
...
- отдельно запускать **webhook-обработчик Битрикс** (для команд старт/пинг) и **dashboard мониторинга**.
...
1. **Mirror service** — основной процесс, который:
   ...
   - запускает фоновый fetch loop, опрашивающий события Битрикс по протоколу API 2.0.
2. **Bitrix webhook service** — HTTP endpoint для базовых команд Битрикс-бота.
...
2. опрашивает новые события методом `imbot.v2.Event.get`;
